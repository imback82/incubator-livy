  ...
  def apply(conf: SparkConf, entries: SparkEntries): SparkDotnetInterpreter = {
    ...
    try {
      ...
      val dotnetExec = conf
        ...
        .getOrElse("dotnet /tmp/netcoreapp2.1/dotnet-script.dll")
	...
	warn(s"Running in yarn mode in livy")
        packageDir = "."
      } else {
        // local mode
      ...
      env.put("spark.master", "yarn");
      env.put("USER", "yarn");
      env.put("HOME", "/home/yarn");
...
class SparkDotnetInterpreter(
  ...
  final override protected def waitUntilReady(): Unit = {
    ...
    //sendRequest("#r \"/tmp/Microsoft.Spark.dll\"")
    sendRequest("#r \"nuget: Microsoft.Spark, 0.2.0\"")
